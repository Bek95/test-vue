...
<template>
  ...
  <div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      <h2>Configuration : {{ selectedType }}</h2>
      <router-link to="/previous-page" class="btn btn-outline-secondary">Annuler</router-link>
    </div>

    <div class="card mb-4 border-primary">
      <div class="card-body">
        <h5 class="card-title">Paramètres du temps</h5>
        ...
        <div v-else-if="['amrap', 'tabata', 'fortime'].includes(selectedType)" class="row">
          <div class="col-md-6">
            <label class="form-label">Durée totale / Time Cap (min)</label>
            <input type="number" v-model="settings.timer" class="form-control">
          </div>
    ...
    <div class="mt-4 text-end">
      <button class="btn btn-success btn-lg">Enregistrer ce bloc</button>
    </div>
  </div>
</template>